...
<table>
  ...
  <tr>
    ...
    <td valign="top">
      <h4>Полка, папки и поиск</h4>
      Обложки с заголовком и началом текста, поиск по тексту работ, корзина. Агент раскладывает полку по папкам сам — «разложи полку» — и сохраняет новое в подходящую папку. Библиотеки шаблонов для команды.
    </td>
    <td valign="top">
  ...
  <tr>
    <td valign="top">
      <h4>Проекты</h4>
      Папка связанных страниц — HTML, Markdown и React-компоненты — становится одной работой: дерево, переходы по ссылкам внутри проекта и одна ссылка на всё.
    </td>
    <td valign="top">
      <h4>Полки отделов</h4>
    ...
    <td valign="top">
      ...
      Правила и модели проверяют каждую ссылку. Прототип с экраном входа или исследование с сотнями источников не задерживаются; получатель видит предупреждение, только если страница просит пароль, код или данные карты.
    </td>
  </tr>
</table>
...
- **Вход — по коду из письма** (нужен SMTP), по паролю от оператора, через Яндекс ID, VK ID, Google или IdP компании по OpenID Connect ([SIGN_IN_PROVIDERS](docs/specs/SIGN_IN_PROVIDERS.md)). Регистрацию можно открыть, ограничить списком адресов, почтовыми доменами или суточным потолком. SAML и SCIM нет.
