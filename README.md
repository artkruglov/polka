...
- **Агент кладёт результат сам.** В Claude.ai и ChatGPT Полка подключается как коннектор: вы пишете «сохрани на Полку» и получаете ссылку в ответе. Claude Code, Codex и другие MCP-клиенты подключаются по токену, скрипты и CI — через HTTP API.
...
> **Статус: prerelease.** Последний тег — `v0.1.0-rc.3`, изменения после него перечислены в [CHANGELOG](CHANGELOG.md). Hosted-пилот работает на https://polochka.app, аккаунты выдаёт оператор. API, схема БД и интерфейс ещё могут меняться. Что сделано и что нет: [docs/status.md](docs/status.md).
...
| Claude Code, Codex, другие MCP-клиенты | Токен со страницы «Агенты», Streamable HTTP на `/mcp` | [Подключение агентов](docs/connect-agents.md) |
...
Поставка состоит из одного Docker-образа и внешних PostgreSQL и versioned S3. Интерактивный viewer должен работать на отдельном registrable domain. Инструкции: [deploy/BASE.md](deploy/BASE.md) (базовая поставка) и [deploy/hosted/README.md](deploy/hosted/README.md) (одна VM с Caddy, как на polochka.app). Как устроена система: [docs/architecture.md](docs/architecture.md).
