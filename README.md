...
| Что есть | Всё открытое ядро | Всё открытое ядро: полки отделов, роли, агенты по полкам, администратор компании, вход через IdP | Ядро и расширение для организаций: ссылки только для сотрудников, политика ссылок, журнал агентов с выгрузкой в SIEM; дальше — интеграции, SAML и SCIM, установка без интернета |
...
> [!NOTE]
> **Текущий релиз — `v0.2.0`** ([CHANGELOG](CHANGELOG.md)). Hosted-пилот работает на https://polochka.app; регистрация по почте открыта, до 50 новых полок в сутки. API, схема БД и интерфейс ещё могут меняться.
