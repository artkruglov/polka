    ...
    <div className="public-detail">
      ...
      <div className="public-detail-grid">
        ...
        <div className="detail-copy">
          ...
          <div className="detail-actions"><button className="primary" onClick={() => setStarted((value) => !value)}><MonitorPlay size={17} /> {started ? "Скрыть демо" : "Открыть демо"}</button><button onClick={() => location.assign("/")} aria-label="Сохранить после входа"><Heart size={17} /> Сохранить после входа</button></div>
...
export function PublicGallery() {
  ...
  if (demo) return <div className="public-page"><header className="public-header"><BrandLink /><div className="public-header-actions"><a href="/">Войти</a><button className="primary" onClick={() => location.assign("/")}><Plus size={16} /> Принести свою работу</button></div></header><DemoDetail demo={demo} onBack={() => open(null)} /></div>;
  return (
    <div className="public-page">
      <header className="public-header"><BrandLink /><nav className="public-nav"><a className="active" href="/discover">Витрина</a><a href="#how">Как это работает</a></nav><div className="public-header-actions"><a href="/">Войти</a><button className="primary" onClick={() => location.assign("/")}><Plus size={16} /> Принести свою работу</button></div></header>
      <main className="public-main">
        <section className="public-hero"><div><span className="eyebrow">ПРИМЕРЫ, КОТОРЫЕ ХОЧЕТСЯ ОТКРЫТЬ</span><h1>Хорошие идеи<br /><em>остаются рядом.</em></h1><p>Презентации, отчёты, дашборды и прототипы, созданные людьми и агентами. Откройте пример, сохраните его для себя или принесите свою работу.</p><div className="hero-actions"><button className="primary" onClick={() => open(demos[0].id)}>Посмотреть примеры <ArrowUpRight size={17} /></button><button onClick={() => location.assign("/")}>Добавить свою работу</button></div></div><div className="hero-stack"><div className="hero-stack-card hero-stack-back"><span>03</span><strong>новый<br />ритм</strong></div><div className="hero-stack-card hero-stack-mid"><span>02</span><strong>пульс<br />команды</strong></div><div className="hero-stack-card hero-stack-front"><span>01</span><strong>города,<br /><em>которые нас</em><br />изменили</strong><small>полка / пример</small></div></div></section>
        ...
        <section className="public-submit" id="how"><div className="submit-icon"><WandSparkles size={26} /></div><div><span className="eyebrow">СВОЯ РАБОТА</span><h2>Есть исследование или проект?</h2><p>Сохраните его на Полке. В ближайшем пилоте можно будет предложить конкретную версию в общую витрину — после короткой проверки.</p></div><button onClick={() => location.assign("/")}>Открыть свою полку <ChevronRight size={17} /></button></section>
      </main>
