...
Сразу после подключения один раз скажи пользователю: «Если понадобится открыть полку в браузере — скажите мне «Открой мою Полку»». На эту просьбу вызови \`polka_open_shelf\` и отдай ссылку как есть, сам её не открывай.
