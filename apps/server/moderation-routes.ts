...
import { ACTION_LABEL } from "./moderation-mail.ts";
import {
...
const body = z.object({ token: z.string().max(600) }).strict();
...
async function describe(token: ModerationToken) {
  const {
    ...
    `SELECT share.id,share.tenant_id,share.moderation,share.moderation_reason,
       ...
       revision.phishing_signals,account.name,account.email,account.disabled,
  ...
  return {
    ...
    share: {
      id: row.id as string,
      title: clean(row.title, 160) || "Без названия",
...
async function perform(token: ModerationToken): Promise<OperatorOutcome> {
  switch (token.action) {
    case "approve":
      return approveShareAsOperator(token.shareId, false);
    case "approve-trust":
      return approveShareAsOperator(token.shareId, true);
    case "unpause":
      return unpauseShareAsOperator(token.shareId);
    case "close":
      return closeShareAsOperator(token.shareId);
    case "close-disable":
      return closeAndDisableAsOperator(token.shareId);
    case "preview":
      throw new Problem(
        400,
        "invalid",
...
export function registerModerationRoutes(app: FastifyInstance) {
  ...
  app.post("/api/moderation/act", { bodyLimit: 2048 }, async (req) => {
    ...
    try {
      const outcome = await perform(token);
      console.info(
        JSON.stringify({
