...
malicious — явный вред: майнинг криптовалюты, выдача исполняемых файлов (.exe, .apk, .bat…), сбор паролей, кодов из SMS и данных карт под видом банка или сервиса, попытки выйти из песочницы (top.location, всплывающие окна без клика) или скрытно отправить наружу то, что вводит посетитель, cookie или данные устройства (WebSocket, sendBeacon, fetch, WebRTC, пиксели), снятие отпечатка браузера, намеренное зависание вкладки, сильная обфускация, скрывающая одно из перечисленного.
suspicious — вред правдоподобен, но не очевиден: например, обфускация, за которой не видно цели, или отправка введённого посетителем на сторонний адрес без явного действия с его стороны.
safe — обычный интерфейс, графики, игры, анимации, минифицированный код библиотек.

НЕ помечай обычное поведение страниц: игры и анимации на requestAnimationFrame; калькуляторы, в том числе вычисляющие выражение самого посетителя; localStorage для состояния страницы; картинки и шрифты в base64; скачивание, которое посетитель сам запросил («Экспорт CSV»); кнопки «Поделиться», открывающие вкладку по клику; запросы к публичным API для показа данных; формы обратной связи, отправляющие то, что посетитель сам решил отправить; Web Workers для тяжёлых, но конечных вычислений; визуализации WebAudio; service worker для офлайн-кэша. Необычный или неэффективный код — не вредоносный.
