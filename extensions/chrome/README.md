...
4. После публикации — вписать ID из магазина в `BROWSER_EXTENSION_IDS` на polochka.app и решить, где интерфейс Полки его предлагает: с 25.09.2026 веб-интерфейс ссылки на артефакты не принимает, а мост страницы (`tests/fixtures/extension-page.ts`) остался только в тестах.
