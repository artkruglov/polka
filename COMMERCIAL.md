...
- **Коммерческая редакция** — расширение ядра для организаций (`@polka/enterprise`), закрытый код. Распространяется только по договору с правообладателем, Кругловым Артемом Игоревичем, и включается подписанным ключом лицензии. Подключается через [точки расширения](docs/specs/EXTENSIONS.md) ядра.
  - Сейчас в ней контроль ссылок (ссылки только для сотрудников, предельный срок ссылок для компании, кто выпускает ссылки с полок отделов) и журнал действий агентов с выгрузкой в SIEM.
  - Дальше: агент только к папке или библиотеке, события и коннекторы (Битрикс24, Jira, 1С), выгрузка в S3 и WebDAV, SAML и SCIM, установка без интернета.
