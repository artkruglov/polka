...
> **Статус:** реализовано, выключено по умолчанию (`URL_IMPORT_ENABLED`). Ссылки Claude/ChatGPT сервером не копируются.
