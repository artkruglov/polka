...
Решение для новой реализации, 14 сентября 2026. Есть отдельный UX-прототип и первый локальный срез; полный HTML/runtime и агентские входы ещё не реализованы. Дизайн опирается на [F01–F30](REQUIREMENTS.md), исходные PRD и проверку зависимостей Lanka. Выбор S3 API ниже не означает выбор AWS как облака или разрешение отправлять туда данные компании.
