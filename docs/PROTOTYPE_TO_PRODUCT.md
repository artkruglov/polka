...
Есть новый [интерактивный макет](design/polka-interface.html) и [первый рабочий срез D1/D2](LIVE_SLICE_2026-09-13.md): React, HTTP API, аккаунты оператора, PostgreSQL/S3, изображения/текст, версии и ссылки. Общая сессия агента, HTML runtime, внешний release и корпоративная поставка ещё не реализованы. Отдельный локальный HTTP-сервер макета не является hosted-версией Полки.
