...
**Реализация:** [первый локальный срез](LIVE_SLICE_2026-09-13.md) уже использует React/API/PostgreSQL/S3 и file-v1. Статический HTML-viewer теперь работает в sandbox без скриптов и сети; схема ниже — целевая, а organization/workspace memberships, HTML runtime/ZIP, workers и внешняя поставка ещё открыты.

Решение для новой реализации, 14 сентября 2026. Есть отдельный UX-прототип и первый локальный срез; статический HTML уже показывается через version-pinned endpoint, полный HTML/runtime, ZIP и агентские входы ещё не реализованы. Дизайн опирается на [F01–F42](REQUIREMENTS.md), [доменную модель](DOMAIN_MODEL.md), исходные PRD и проверку зависимостей Lanka. Этапы M1.2/M2 в тексте ниже соответствуют новым M1 Light и отложенному редактору ([план](DELIVERY_PLAN.md#дорожная-карта-с-14-сентября-2026)). Выбор S3 API ниже не означает выбор AWS как облака или разрешение отправлять туда данные компании.

Production-развёртывание Light в Яндекс Облаке, capacity-гипотеза для 50 000 пользователей, Telegram, social discovery и self-hosted/open-source профили собраны в [production-плане](../../reviews/2026-09-14-opus-production-plan/review.md) и уточнены [Fable review](../../reviews/2026-09-14-fable-production-review/review.md). Для релиза надёжным P0 считается HTML/ZIP и MCP capture; внешний URL — best-effort.
...
| Сущности сообщества (M1.1–M1.3) | Origin, PublicationSnapshot, ModerationCase, Bookmark, Reaction `useful`, Collection, AgentConnection, Capability, Delegation, ActionIntent, ApprovalReceipt, AgentHandoff, ViewEvent — поля, инварианты и границы приватности в [DOMAIN_MODEL](DOMAIN_MODEL.md). Tenant получает `kind = personal \| team \| organization`; ShareLink/Grant в продуктовых документах называется ShareGrant |
| Следующие сущности | CommentAnchor, Proposal, Approval (M3/M4); ReleaseManifest, DataSnapshot/Source, DesignPackageRelease — отложены |
...
**Обновлено 2026-09-14.** Публичная проекция строится только из PublicationSnapshot `published` отдельными запросами и кешами; ModerationCase объединяет заявки, жалобы и апелляции; Bookmark/Reaction/Origin — отдельные таблицы с уникальными ключами; ремикс копирует blobs публичного bundle в tenant ремиксера (без межтенантной дедупликации, см. §2). Team tenant в Light использует тот же код, что organization в Enterprise, с урезанными ролями и без SSO/egress UI. Light и Enterprise не синхронизируются; перенос — подписанный пакет P33. Детали — [DOMAIN_MODEL](DOMAIN_MODEL.md).
