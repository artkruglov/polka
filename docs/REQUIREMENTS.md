...
| F06 | Полка/папки: создать, переименовать, переместить, удалить/восстановить, сортировать, найти, открыть. Цикл папок и перенос между tenants запрещены обычным move; новые effective permissions пересчитываются | M1 / P06 |
| F07 | Постоянный непрогнозируемый share address указывает на exact revision по умолчанию; выдаваемые URL данных короткоживущие. Есть аудитория, expiry, revoke, отдельные download/copy. Отзыв не обещает удалить уже скачанные байты | M1 / P07 |
...
| F21 | Поиск, обложки, кеши и счётчики соблюдают текущие ACL. Дедупликация не раскрывает наличие чужого файла; invalidation учитывает move/revoke/offboarding. Full-text/vector search не обязательны в M1 | Metadata M1, расширение M5 / P21 |
