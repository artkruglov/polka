...
Сначала [витрина примеров](DEMO_LIBRARY.md): M1 — шесть живых демо без регистрации; M2 — взять за основу и сохранить пример в закладки; M4 — библиотека команды; M5 — пользовательские публикации и социальные сигналы. Для редакционной витрины не требуется ждать M5, но viewer каждого примера должен быть принят P08.
