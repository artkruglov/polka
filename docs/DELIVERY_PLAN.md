...
4. M1.1: S12/P07/P21/P22 + P18 → публичная проекция, авторская заявка, ручная очередь, общий каталог, снятие/жалобы и простые реакции. После эксплуатационного допуска M1; не ждать корпоративного пилота и не ослаблять изоляцию HTML.
5. P09 + copy P17 → собственный результат B. Проверять отдельно от A.
6. P20: внешний агент сохраняет/читает candidate и получает owner-only адрес. Затем P13/P14: поручение из UI, обсуждение и исправление.
7. S11/P15/P16/расширенный P23 — корпоративный пилот. M5/P22 — расширенные подписки/обсуждение/рекомендации по использованию; ранние публичные работы уже входят в M1.1.
