...
5. Страница без интерактивной версии показывается в статичной песочнице: CSP без скриптов и без сети. Если viewer настроен, статичный просмотр тоже приходит с него (`/static/:token`, грант на 60 секунд), и домен приложения пользовательский HTML не отдаёт. Только установка на одном домене показывает его с домена приложения (`/api/…/document`).
6. Внешние `http(s)`-ссылки статичного просмотра при отдаче заменяются на `APP_ORIGIN/away#<токен>`. Токен — HMAC адреса и срока на ключе, выведенном из `LINK_KEY`. Страница `/away` называет адрес и ждёт нажатия, без подписи ничего не открывает. Интерактивный просмотр ссылок не открывает: у песочницы нет `allow-popups`.

## Границы доверия

| Граница | Как защищена |
|---|---|
| Пользовательский HTML ↔ Полка | Отдельный registrable domain для интерактивного и статичного просмотра, `sandbox` без `allow-same-origin`, строгая CSP без сети, короткоживущий грант на конкретную версию. Внешние ссылки статичного просмотра идут через подписанную страницу `/away`. Контракты: [LIVE_VIEWER_SPEC](LIVE_VIEWER_SPEC.md), [HOSTED_VIEWER_DELTA](HOSTED_VIEWER_DELTA.md) |
...
| Открытая регистрация ↔ получатели | Новый аккаунт — до 5 открытых ссылок по 7 дней; `SHARE_MODERATION` задерживает ссылку до проверки (страница похожа на фишинг или автор новый), жалобы разных получателей ставят её на паузу. Получатель такой ссылки не получает ни названия, ни гранта. Оператор решает по письму: кнопка — подписанный токен во фрагменте `/moderation#…`, открытие ничего не меняет, действие — только POST. См. [ABUSE_PROTECTION](specs/ABUSE_PROTECTION.md) |
...
| `content/editorial` | Исходники материалов «Ленты»: 18 в каталоге (`candidates.json`), черновики — `owner-research-candidates.json` |
