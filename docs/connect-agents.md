...
**Вернуться в полку из браузера: «Открой мою Полку».** Агент, подключённый через OAuth (Claude.ai, ChatGPT, Claude Code, Codex), по этой просьбе вызывает `polka_open_shelf` и даёт одноразовую ссылку `https://polochka.app/enter#…`: она открывает в браузере ту полку, куда агент сохраняет работы. Ссылка действует 5 минут и один раз; у подключения со статическим токеном её нет. Выключить — на странице «Агенты», у подключения: «Может выдавать ссылки для входа».
...
По этой ссылке лежит инструкция для агентов. Агент сам выполнит нужную команду, откроется Полка: войдите в свою полку (или нажмите «Начать без регистрации») и нажмите «Разрешить». Токены и пароли через агента не проходят.
...
| `polka_open_shelf` | `context`, только OAuth | Одноразовая ссылка для входа в эту полку в браузере (5 минут); по просьбе «Открой мою Полку» |
