...
> **Status: prerelease.** The latest tag is `v0.1.0-rc.1`. Changes since then are listed in the [CHANGELOG](CHANGELOG.md). A hosted pilot runs at https://polochka.app, and the operator creates the accounts. The API, database schema and UI may still change. What works and what doesn't: [docs/status.md](docs/status.md) (Russian).
