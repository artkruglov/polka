...
- агента можно подключить без регистрации: откроется временная полка этого браузера. Ссылки она не выдаёт, пока её не закрепят Яндекс ID, VK ID или почтой на российском домене. Неиспользуемую 30 дней удаляет обслуживание — через обычный конвейер удаления, поэтому нужен `ACCOUNT_DELETION_ENABLED=true`, как в этой форме установки;
- агент, подключённый через OAuth, по просьбе «Открой мою Полку» выдаёт одноразовую ссылку для входа (5 минут). Владелец отключает это у подключения в разделе «Агенты».

**Объединить две полки одного человека.** Сначала пробный прогон — он ничего не меняет и печатает, что переедет:
...
docker compose --env-file hosted.env exec -T app node --import tsx scripts/account-merge.ts \
  --from <…> --into <…> --reason "письмо владельца от 24.09"
```
...
- источник отключается, его сессии завершаются, в журнал пишется `account.merged` (`moderation.ts events <id источника>`).
...
Адрес почты источника переезжает, только если у основной полки почты нет. Иначе он остаётся у отключённого источника, и вход по коду на него не работает; скрипт об этом пишет. Локально: `npm run account:merge -- --from … --into … --dry-run`.
