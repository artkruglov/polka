...
| `recipient_cta_view` | гость страницы по ссылке (`/s`) увидел подсказку «сделали с ИИ и сохранили на Полку»: `surface` = `bar` (полоса, раз на загрузку) или `card` (карточка). Только без сессии и с браузерным User-Agent; без получателя и без ссылки (миграция 035, `apps/server/recipient-cta.ts`) |
| `recipient_cta_click` | нажатие в подсказке: `action` = `try`, `remix`, `copy_phrase`, `yandex`, `email`. Регистрация, начатая оттуда, приходит с источником `ref:share` или `ref:share-remix` |
