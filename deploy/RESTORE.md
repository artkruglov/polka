...
Prepare an exact backup descriptor file and a separate erasure-ledger manifest authority. The descriptor must be a regular file, be no larger than the runner limit, declare `formatVersion: 1`, the exact migration list for this release (the exact set is in `packages/migrations.ts`, currently 030), the expected `erasureLedgerId`, and `localMailSpool: "absent"`. The operator must independently attest that the target host has no local mail spool; the descriptor field is checked but does not inspect the host filesystem.
