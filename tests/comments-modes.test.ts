...
async function link(owner: Person) {
  ...
  const begin = await call(
    "POST",
    "/api/uploads",
    { key: randomUUID(), title: "План", filename: "page.html", mime: "text/html", size: bytes.length, sha256: sha256(bytes) },
    owner.cookie,
  );
...
test("owner-notes: recipients read the owner's notes and cannot write or react", async () => {
  ...
  const earlier = await call("POST", "/api/shared/comments/create", { token: work.token, body: "Старый комментарий получателя" }, reader.cookie);
  ...
  const viaLink = await call("POST", "/api/shared/comments/create", { token: work.token, body: "Ещё заметка" }, owner.cookie);
  ...
  const guest = await call("POST", "/api/shared/comments/create", { token: work.token, body: "Гость" });
  ...
  const ownerReact = await call("POST", `/api/artifacts/${work.artifactId}/reactions`, { shareId: work.shareId, emoji: "👍" }, owner.cookie);
...
test("owner-notes: the owner's agent notes the newest open link", async () => {
  ...
  const seen = await call("POST", "/api/shared/comments", { token: work.token });
  assert.deepEqual(seen.json().threads.map((thread: any) => thread.body), ["Агент: поправить второй шаг"]);
