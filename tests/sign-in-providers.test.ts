...
const codes = new Map<
  string,
  { person: Person; challenge: string; nonce?: string; forgeSignature?: boolean }
>();
...
before(async () => {
  server = createServer(async (req, res) => {
    ...
    if (url.pathname === "/vk/token" && req.method === "POST") {
      ...
      const issued = issueToken(form.code, form.code_verifier);
      if (!issued) return send(400, { error: "invalid_grant" });
      return send(200, { access_token: issued.access, user_id: Number(issued.grant.person.sub), state: form.state });
    }
    if (url.pathname === "/vk/user_info" && req.method === "POST") {
      ...
      if (!person) return send(401, {});
      return send(200, {
        user: { user_id: person.sub, first_name: "Вера", last_name: "Кузнецова", email: person.email },
      });
    }
...
test("capabilities list configured providers; a disabled provider has no routes", async () => {
  ...
  try {
    const off = await app.inject({ method: "GET", url: "/api/auth/idp/vk/start" });
    assert.equal(off.statusCode, 404);
  } finally {
    config.SIGN_IN_PROVIDERS = ["yandex", "vk", "oidc"];
  }
  assert.equal(
    (await app.inject({ method: "GET", url: "/api/auth/idp/google/start" })).statusCode,
    404,
  );
});
...
test("Яндекс ID: a new person gets a shelf and returns to the agent consent page", async () => {
  ...
  const me = await app.inject({ method: "GET", url: "/api/me", headers: { cookie: `polka_session=${result.session}` } });
  assert.equal(me.statusCode, 200);
  const again = await authorize("yandex", { sub, email: `anna.${sub}@yandex.ru` });
...
test("a foreign state, a missing or forged cookie and a replayed callback are refused", async () => {
  ...
  const wrongState = await authorize("yandex", person, { state: () => randomBytes(32).toString("base64url") });
  assert.match(wrongState.location, /^\/signup\?idp_error=state/);
  assert.equal(wrongState.session, undefined);
  const noCookie = await authorize("yandex", person, { cookie: () => undefined });
...
test("a verified address links the existing shelf; an unverified one never does", async () => {
  ...
  const vk = await authorize("vk", { sub: String(Math.floor(Math.random() * 1e9)), email });
  ...
  const oidc = await authorize("oidc", { sub: `o-${randomUUID()}`, email, email_verified: false });
...
test("OIDC: nonce and signature are checked; allowed domains and the org claim restrict sign-in", async () => {
  ...
  const ok = await authorize("oidc", { sub: `o-${randomUUID()}`, email, email_verified: true, name: "Иван" });
  ...
  try {
    const outside = await authorize("oidc", {
      sub: `o-${randomUUID()}`, email: "someone@elsewhere.test", email_verified: true, groups: ["polka-users"],
    });
    assert.match(outside.location, /idp_error=domain/);
    const notInGroup = await authorize("oidc", {
      sub: `o-${randomUUID()}`, email, email_verified: true, groups: ["other"],
    });
    assert.match(notInGroup.location, /idp_error=domain/);
    const member = await authorize("oidc", {
      sub: `o-${randomUUID()}`, email: `m-${randomUUID().slice(0, 6)}@company.test`, email_verified: true, groups: ["polka-users"],
    });
...
test("invite mode and the daily budget apply to new shelves from providers", async () => {
  ...
  try {
    const refused = await authorize("yandex", { sub: `i-${randomUUID()}`, email: `x-${randomUUID().slice(0, 6)}@yandex.ru` });
    assert.match(refused.location, /idp_error=signup/);
    config.EMAIL_SIGNUP_ALLOW = ["@invited.test"];
    const invited = await authorize("yandex", { sub: `i-${randomUUID()}`, email: `x-${randomUUID().slice(0, 6)}@invited.test` });
...
test("linking from settings binds the provider to the signed-in shelf, once", async () => {
  ...
  const second = await authorize("yandex", { sub, email: `${sub}@yandex.ru` }, {
    start: { location: new URL(again.json().location), flow: cookieFrom(again, "polka_idp")!, ip: address() },
  });
  assert.match(second.location, /idp_error=linked/);
  // Settings list it; unlinking works while the email remains a way in.
  const listed = await app.inject({ method: "GET", url: "/api/account/identities", headers: { cookie: owner.cookie } });
  assert.deepEqual(listed.json().identities.map((item: any) => item.provider), ["yandex"]);
...
test("organisation access: a verified domain joins the library once; a revoked member stays out", async () => {
  ...
  try {
    ...
    assert.deepEqual(event, { action: "template_library.domain_joined", new_role: "reader" });
    // An unverified address of the same domain (VK) joins nothing.
    const unverified = await authorize("vk", { sub: String(Math.floor(Math.random() * 1e9)), email: `u@${domain}` });
    ...
    assert.deepEqual((await members()).map((row: any) => row.state), ["revoked"]);
